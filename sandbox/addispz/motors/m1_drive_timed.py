...
def main():
    ...
    while time_s != 0:
        motors_sp = int(input("Enter a speed (0 to 900 dps): "))
        distance_s = int(input("Distance to travel (inches): "))
        distance_t = (distance_s / motors_sp * 0.0112)
        left_motor.run_forever(speed_sp=motors_sp)
        right_motor.run_forever(speed_sp=motors_sp)
        time.sleep(distance_t)
        left_motor.stop()
